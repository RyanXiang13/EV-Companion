...
/**
* This Application is used to provide the user with detail cost analysis between
* a electrical vehicle and a gas-powered vehicle based in the users input for their  
* expected vehicle price, anual travel distance and prefered vehicle type. 
*
* @author  Ryan Xiang
...
class Code {
  ...
  private static final int MENU_APPLICATION_LOAD = 0;  
  ...
  private static final int MENU_VEHICLE_TYPE_MINIVAN = 2;
  ...
  private static final int MENU_PRICE_RANGE_40K_60K = 2;
  private static final int MENU_PRICE_RANGE_OVER_60K = 3;
  ...
  private static final int MENU_ANUAL_KILOMETRAGE_10K_15K = 2;
  private static final int MENU_ANUAL_KILOMETRAGE_15K_20K = 3;
  private static final int MENU_ANUAL_KILOMETRAGE_20K_25K = 4;
  private static final int MENU_ANUAL_KILOMETRAGE_25K_30K = 5;
  private static final int MENU_ANUAL_KILOMETRAGE_30_35K = 6;
  private static final int MENU_ANUAL_KILOMETRAGE_BACK = 7;
  private static final int MENU_ANUAL_KILOMETRAGE_EXIT = 8;
  
  // Declare constant String variables for Electrical/Gas vehicles to avoid magin numbers
  ...
  // Initialize two dimensional array to all menus and their corresponding menu items
  ...
  */  
  public static void main(String[] args) {
    ...
    // While loop is used to all the users to repeatedly navigate through menus, make selection and
    // display the cost results, unless the selection to exit the application execution is selected.    
    while (true) {
      ...
      if (curMenu == MENU_VEHICLE_TYPE) {
        ...
        // Prompt the users to re-enter a valid selection if they have entered an invalid one
        ...
        // Display and store the user input from this menu 
        ...
        // If the users selected to go back to prev. menu, decrement 'curMenu' by 1 to navigate back
        ...
        // A valid selection for this menu is entered if the code execution reaches here.
        // Display and store the user input from this menu         
        System.out.println("You have selected: " + getSelecteditemText(curMenu, menuSelection));        
        ...
        // A valid selection for this menu is entered if the code execution reaches here.
        // Display and store the user input from this menu              
        input_summary[curMenu] = "-\tEstimated Anual kilometrage: " + getSelecteditemText(curMenu, menuSelection);
        System.out.println("You have selected: " + getSelecteditemText(curMenu, menuSelection));
        System.out.println("");
        ...
        // Invoke the method 'findEVByTypeAndPriceRange' to get the EVs matching the user selected
        ...
        // This is the last menu that the users need to make selection. So now display all 
        // the selections the users have made before proceed for a cost summary report.
        ...
        System.out.println("Or press 2 to go back to modify your selection.");
        ...
        if (decisionSelection == 1) {
          ...
          // Invoke the method 'recordOrderPreference' to ask for users preference in UV vs GV, 
          // and record the user response together with user selection input into database. 
          // The data may be used by government or non-profit agencies to analysze emission 
          // prograss status from antomobile section and relevant incentive or rebate policy
          ...
          System.out.print("Press 1 to continue, or press 2 to exit the program.");
          decisionSelection = Integer.parseInt(input.next());
          System.out.println("");

          if (decisionSelection == 1) {
            // The user has selected to continue with another cost assessment
            // Reset navigation to the first menu and continue
            curMenu = 1;
            continue;
          } else {
  ...
  /**
  *
  * This method is used to load all menu items into a two dimensional array.
  * It helps to retrieve the menu item contents easily with number entered by the users.
  */
  public static void populateMenuDataMapping() {

    // Assign the global two dimensional array with all the statice menu option items. The
    // row index refers to the menu and the col index refer to the menu items within the menu.
  ...
  *
  * @param a  The menu to be displayed
  *
  */
  public static void displayMenu(String selectedMenu) {

    // From the all menuStr 'menuStr', find the start and end position of the menu 'selectedMenu' to
    // be displayed. 
    ...
    // Plug in actual formatting code as per specified in the 'menuStr'. Ex. the [BlueColor]
    // specified in the 'menuStr' (from menu.txt file) will be replaced by actual format code
    // for blue font "\033[94m".    
  ...
  */
  ...
  /**
  * This method is used search for electrical vehicles matching the vehicle type and price
  * range stored in the datafile "EV_inventory.txt". 
  *
  * @param vehicleType  The type of vehicle (like Sedan, Minivan ..)
  * @param b  The minimum price of the vehicle the users are looking for 
  * @param b  The minimum price of the vehicle the users are looking for
  *
  ...
  */
  public static ArrayList<String> findEVByTypeAndPriceRange(String vehicleType, double min, double max) {
    ...
    // Initialize a ArrayList of Strings to store string representation of vehicles matching serarch
    // criteria for vehicle type and min and max price range
    ...
    try {
      ...
      while (fileInput.hasNextLine()) {
        ...
        // Add to the item to the list 'matchItems' if it matches the search criteria for
        // vehicle type and price range
  ...
  /**
  * This method is used to parse the price range from the user input and return the
  ...
  */  
  public static double[] getPriceRange() {
    ...
    // Assign 'minPrice' and 'maxPrice' to the 1st and 2nd elements of 'minMax[]' respectively
    minMax[0] = minPrice;
    minMax[1] = maxPrice;
  ...
  /**
  *
  * This method is used to parse the kilometrage range from the user input and return the
  * result to the caller. 
  *
  ...
  */  
  public static double[] getkilometrageRange() {
    ...
    // Get the string representation of Kilometrage range stored in 'input_summary'
    String priceRange = input_summary[MENU_ANUAL_KILOMETRAGE];

    // Initialize the min / max prices    
    double minkilometrage = 0;
    double maxkilometrage = Double.MAX_VALUE;

    // Parsing out min and max Kilometrage
    minMax[0] = Double.parseDouble(input_summary[MENU_ANUAL_KILOMETRAGE].split("-")[1].trim().replaceAll("[^0-9.]", ""));
    minMax[1] = Double.parseDouble(input_summary[MENU_ANUAL_KILOMETRAGE].split("-")[2].trim().replaceAll("[^0-9.]", ""));
  ...
  * @return double  This returns sum of a and b.
  */  
  ...
  /**
  * This method is used to load and display dynamic menu requested by the invoker.. 
  *
  * @param selectedMenu  the menu to load and displayed
  ...
  */
  public static void loadDynamicMenu(int selectedMenu, ArrayList<String> menuItems) {
    ...
    if (selectedMenu == MENU_PREFERED_EV_TYPE) {
      // To load / display the menu for the EVs matching the user's vehicle type and 
      // price range
      ...
      for (int i = 0; i < menuItems.size(); i++) {
        ...
        System.out.println("\t" + menuItemNo++ + "\t" + temp[1] + "\t" + formattedPrice);
  ...
  */  
  public static void calculateCostData() {
    ...
    // Invoke method 'findVehicleByTypeAndName' to retrieve the complete information of 
    // the equivalent GV by EV' type and name    
    ...
    // Calculate the average anual kilometrage by average over the min / max mkilometrages
    double[] minMaxkilometrage =  getkilometrageRange();
    double avgkilometrage = (minMaxkilometrage[0] + minMaxkilometrage[1]) / 2;

    // Calculate the average anual fuel cost for the selected EV and kilometrage
    ev_fuel_cost = avgkilometrage * evFuelCostPer100km / 100;
    // Calculate the average anual fuel cost for the equivalent EV and kilometrage    
    ...
    // Calculate the total anual EV upkeep cost. The battery replacement is distributed evenly
    // with the EV Battery warranty, ususally 8 years for now
    ev_upkeep_cost = batteryReplCost / EV_BATTERY_WARRANTY_YEAR + otherEVCostPer100km;

    // Parse out the upkeep base rate of the equivalent GV
    double gvUpkeepBase = Double.parseDouble(gv_data.split(",")[5]);
    // Invoke 'getUpkeepCost' method recursively to calculate the total upkeep cost in 8 year 
    // used in evaluate corresponding EV upkeep cost
    double todal_upkeep_cost = getUpkeepCost(1, 8, EV_BATTERY_WARRANTY_YEAR, avgkilometrage/100);
    // Average out the total GV upkeep cost over 8 years to get estimated anual GV upkeep cost
    match_gv_upkeep_cost = todal_upkeep_cost / EV_BATTERY_WARRANTY_YEAR;
  }


  /**
  * This method is to caculate the upkeep cost of a gas powered vehicle recusively, 
  * for each year from the nth year specified by parameter 'nthYEar', till the total 
  * number of years specfied by parameterd 'totalYears' are calculated. 
  *
  * @param nthYear  the nth year of which the upkeep cost is calculated 
  * @param totalYears  total number of years the upkeep cost is calculated 
  * @param baseCost  The baseCost of the first year
  * @param kilometrage The anual kilometerage
  
  * @return double the total cost from nth year to (totalYear - nth ) years
  *
  */  
  private static double getUpkeepCost(int nthYear, int totalYears, double baseCostRate, double kilometrage) {

    // Base case when it comes to the last year of cost calculation, terminate
    // the recursive call by returning the cost of current year
    if (nthYear == totalYears) {
      return baseCostRate * kilometrage;
    }

    // Otherwise, add up cost of current year to the cost of next year, which is one year closer
    // to the final year.
    return baseCostRate*kilometrage + getUpkeepCost(nthYear+1, totalYears, 
                                    baseCostRate + (getGVUpkeepCostCoeffByYear(nthYear)), kilometrage);
  }


/**
  *
  * This method is used to calculate GV upkeep cost coefficient
  ...
  */  
  public static double getGVUpkeepCostCoeffByYear(int numberOfYears) {
    ...
    if (numberOfYears <= 3)
      // For the first three years, the coefficient is 1.0
      coefficient = 1.0; 
    else if (numberOfYears > 3 && numberOfYears <= 5) {
      // In year 4 and year 5, the coefficient is 2.0
      coefficient = 2.0; 
    }
    else if (numberOfYears > 5 && numberOfYears <= 8) {
      // After year 5 till year 8, the coefficient is 3.0
      coefficient = 3.0; 
    }
    else {
      // After 8 years, the coefficient increase to 5.0
      coefficient = 5.0 ;
    }

    // Return the cod=efficient to the caller
    return coefficient;
  }   
  ...
  */  
  public static void displayCostSummary() {
    ...
    sleep(100);
    System.out.println("\tYou are entitled to get " + getCurrencyFormattedNum(ev_rebate, 0) + " rebate from government.");
    sleep(100);
  ...
  /**
  *
  * This method is to wrap java sleep method in try catch block so we don't need to 
  * wrtie try..catch with the built-in java sleep everywhere we use it. 
  *
  ...
  */  
  ...
  /**
  *
  * This method is used to retrieve the vehicle information based on the vehicle category (GV or EV),
  * vehocle type (sedan, minivan) and add vehicle name as passed into the method as parameters. 
  *
  * @param vehicleCategory  This is the category of vehicle such as electrical vehicles, gas vehicles
  * @param vehicleType  This is the type of vehicle, such as Sedan, minivan
  * @param vehicleType  This is the name of the vehicle
  *
  * @return String This returns the full information of the vehicle determined by the category,
  * type and name of the vehicle.
  *
  */  
  public static String findVehicleByTypeAndName(String vehicleCategory, String vehicleType, String vehicleName) {
    ...
    try {
      ...
      // Determin appropriate data file to load based on the 'vehicleCategory' passed in.
      ...
      while (fileInput.hasNextLine()) {
        ...
        // If match found, store the 'line' into var 'matchVeh' to return later, and then break out from
        // the search loop
        if (myVehicleType.compareToIgnoreCase(vehicleType) == 0 && myVehicleName.compareToIgnoreCase(vehicleName) == 0) {
          matchVeh = line;
          break;
        }
      }
